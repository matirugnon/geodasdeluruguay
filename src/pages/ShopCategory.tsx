                        ...
                        {products.map(p => (
                            ...
                            >
                                <div className="relative w-full aspect-[4/5] overflow-hidden rounded-xl bg-surface-light dark:bg-surface-dark">
                                    ...
                                    ></div>
                                    <button
                                        className="absolute top-3 right-3 p-2 bg-white/80 dark:bg-black/50 rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-primary hover:text-white z-10"
                                    ...
                                    <button
                                        ...
                                        className={`absolute bottom-3 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-4 py-2 rounded-full text-xs font-bold transition-all duration-300 shadow-md whitespace-nowrap ${addedIds.has(p.id)
                                                ? 'bg-stone-800 text-white opacity-100 translate-y-0'
                                                : 'bg-white/90 text-stone-700 hover:bg-primary hover:text-white opacity-0 group-hover:opacity-100'
                                            }`}
                                        aria-label="Agregar al carrito"
                                    >
                                        <span className="material-symbols-outlined !text-[16px]" style={{ fontVariationSettings: addedIds.has(p.id) ? "'FILL' 1" : "'FILL' 0" }}>
                                            {addedIds.has(p.id) ? 'check' : 'shopping_bag'}
                                        </span>
                                        {addedIds.has(p.id) ? '¡Agregado!' : 'Agregar al carrito'}
                                    </button>
                                </div>
