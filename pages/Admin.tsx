                        ...
                        <form onSubmit={handleSubmitProduct} className="p-6 md:p-8 space-y-8 font-noto-sans">
                            ...
                            <div>
                                ...
                                <div className="flex gap-4 items-end">
                                    <div onClick={addImage} className="h-32 w-32 border-2 border-dashed border-stone-300 rounded-lg flex flex-col items-center justify-center cursor-pointer hover:bg-stone-50 dark:hover:bg-white/5">
                                        <span className="material-symbols-outlined text-stone-400">add_photo_alternate</span>
                                        <span className="text-xs text-stone-500">Añadir URL</span>
                                    </div>
                                    <div className="flex gap-2 overflow-x-auto">
                                        {formData.images?.map((img, idx) => (
                                            <div key={idx} className="relative h-32 w-32 rounded-lg overflow-hidden border border-stone-200">
                                                <img src={img} className="h-full w-full object-cover" />
                                                <button type="button" onClick={() => removeImage(idx)} className="absolute top-1 right-1 bg-white rounded-full p-1 text-red-500 shadow-sm"><span className="material-symbols-outlined text-sm">close</span></button>
                                            </div>
                                        ))}
                                    </div>
                                </div>
